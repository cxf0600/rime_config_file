...
--引入系统变更处理模块
local sysInfoEnable, sysInfo = pcall(require, 'Litles/sysInfo')

--引入 utf8String 处理字符串相关操作
local utf8StringEnable, utf8String = pcall(require, 'Litles/utf8String')

local logEnable, log = pcall(require, "Litles/runLog")
if logEnable then
	log.writeLog('')
	log.writeLog('log from customPhrase_dict.lua')
...
function M.init(...)
	...
	table.insert(files,"customPhrase/common.txt")
	table.insert(files,"customPhrase/private.txt")
